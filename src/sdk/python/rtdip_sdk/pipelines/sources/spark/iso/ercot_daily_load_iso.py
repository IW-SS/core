...
class ERCOTDailyLoadISOSource(BaseISOSource):
    """
    The ERCOT Daily Load ISO Source is used to read daily load data from CAISO API.
    ...
    required_options = ["load_type", "date", "certificate_pfx_key", "certificate_pfx_key_contents"]
    ...
    def __init__(self, spark: SparkSession, options: dict) -> None:
        ...
        self.certificate_pfx_key_contents = self.options.get("certificate_pfx_key_contents", "").strip()
    ...
    def generate_temp_client_cert_files_from_pfx(self):
        ...
        key, cert, additional = pkcs12.load_key_and_certificates(data=pfx, password=password)
    ...
    def generate_urls_for_zip(self, url: str, date: datetime) -> (List[str], List[str]):
        ...
        for i in range(0, length):
            zip_name = page_content.find_all("td", {"class": "labelOptional_ind"})[i].text
            zip_link = page_content.find_all("a")[i].get("href")
            zip_info.append((zip_name, zip_link))

        date_str = date.strftime("%Y%m%d")
        zip_info = list(filter(lambda f_info: f_info[0].endswith("csv.zip") and date_str in f_info[0], zip_info))
        ...
        for info in zip_info:
            file_name, file_url = info
            urls.append(self.url_prefix + file_url)
            files.append(file_name)
        #
        # for idx in indices:
        #     url_ending = rel_links[idx]
        #     url = self.url_prefix + url_ending
        #     urls.append(url)
        #     files.append(rel_names[idx])
        return urls, files
